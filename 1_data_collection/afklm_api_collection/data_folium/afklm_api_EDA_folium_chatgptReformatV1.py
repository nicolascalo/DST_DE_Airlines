...
for csv_file in call_parameter_csv_list:
    ...
    df_call_parameters = pd.concat(
        [df_call_parameters, df_add], ignore_index=True
    ).fillna("").sort_values(["startRange", "endRange"])
...
df_airports_country_origin.columns = [
    "origin_continent", "origin_subcontinent", "origin_country", "origin",
    "origin_airport", "origin_latitude", "origin_longitude"
]

df_airports_country_destination = df_airports[
    ["continent", "subcontinent", "country", "iata", "airport", "latitude", "longitude"]
].copy()
...
# === Add airport descriptions and connections ===
...
for _, record in df_airport.iterrows():
    ...
    df_filtered = df_airport[df_airport["iata"].isin(connection_iata) & (df_airport["iata"] != record.iata)]
...
df_valid_airports = set(
    df_call_parameters[df_call_parameters["dailyFlights"] > 0][["origin", "destination"]].to_numpy().flatten()
)
df_airport = df_airport[df_airport["iata"].isin(df_valid_airports)]

# === Create map ===

m = folium.Map(crs="EPSG3857", location=[50, 15], zoom_start=2, min_zoom=2, max_bounds=True)
...
for _, row in df_sum.iterrows():
    ...
    polyline = folium.PolyLine(
        lonlats, color=color, opacity=opacity,
        popup=folium.Popup(row.itinerary, max_width=4000), weight=2.5
    )
