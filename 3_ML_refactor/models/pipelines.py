...
from models.grid_params import get_grid_params
from utils.preprocessing_utils import get_numeric_categorical
...
def build_pipelines(datasets, settings):
    ...
    preproc_cls_status = build_preprocessor(datasets['status']['X_train'], settings['TARGET_CLASSIFICATION_STATUS'])
    classification_pipelines = {
        'DecisionTree': [Pipeline([('preprocessor', preproc_cls_status), ('classifier', DecisionTreeClassifier())]), get_grid_params('DecisionTree', 'classification_status')],
        'RandomForest': [Pipeline([('preprocessor', preproc_cls_status), ('classifier', RandomForestClassifier(n_jobs=settings['PARALLEL_JOBS']))]), get_grid_params('RandomForest', 'classification_status')],
        'Logistic_OVO': [Pipeline([('preprocessor', preproc_cls_status), ('classifier', OneVsOneClassifier(LogisticRegression(max_iter=1000), n_jobs=settings['PARALLEL_JOBS']))]), get_grid_params('Logistic_OVO', 'classification_status')]
    }
    ...
    classification_pipelines = {k:v for k,v in classification_pipelines.items() if k in settings['MODEL_LIST_TO_TEST']}

    return {'regression': regression_pipelines, 'classification': classification_pipelines}
