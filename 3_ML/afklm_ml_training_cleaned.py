...
def create_regression_pipeline(model_name, preprocessor, run_mode, n_jobs):
    """Return a regression pipeline and its grid params based on model name and run mode."""
    model_mapping = {
        'LinearRegression': LinearRegression,
        'DecisionTreeRegressor': DecisionTreeRegressor,
        'RandomForestRegressor': lambda: RandomForestRegressor(n_jobs=n_jobs if run_mode == 'simple' else None),
        'XGBRegressor': lambda: XGBRegressor(n_jobs=n_jobs if run_mode == 'simple' else None)
    }
    
    model_cls = model_mapping[model_name]
    pipeline = Pipeline([
        ('preprocessor', preprocessor),
        ('classifier', model_cls() if callable(model_cls) else model_cls)
    ])
    grid_params = get_grid_params(model_name, 'regression')
    return pipeline, grid_params


def create_classification_pipeline(model_name, preprocessor, run_mode, n_jobs):
    """Return a classification pipeline and its grid params based on model name and run mode."""
    if model_name == 'Logistic_OVO':
        classifier = OneVsOneClassifier(
            LogisticRegression(max_iter=1000),
            n_jobs=n_jobs if run_mode == 'simple' else None
        )
    elif model_name == 'Logistic_OVR':
        ...
        classifier = RandomForestClassifier(n_jobs=n_jobs if run_mode == 'simple' else None)
    else:
    ...
    grid_params = get_grid_params(model_name, 'classification_status')
    return pipeline, grid_params



regression_models_valid = [
    "LinearRegression",
    "DecisionTreeRegressor",
    "RandomForestRegressor",
    "XGBRegressor"
...
regression_models_to_test = [m for m in ml_training_settings['MODEL_LIST_TO_TEST'] if m in regression_models_valid]
classification_models_to_test = [m for m in ml_training_settings['MODEL_LIST_TO_TEST'] if m in classification_models_valid]



regression_pipelines = {
    model_name: create_regression_pipeline(model_name, preprocessor_reg, ml_training_settings['RUN_MODE'], ml_training_settings['PARALLEL_JOBS'])
    for model_name in regression_models_to_test
}

classification_pipelines = {
    model_name: create_classification_pipeline(model_name, preprocessor_cls_status, ml_training_settings['RUN_MODE'], ml_training_settings['PARALLEL_JOBS'])
    for model_name in classification_models_to_test
}
# ----------------------
