...
os.makedirs(f'{OUTPUT_DIR}/classification/models', exist_ok=True)
...
os.makedirs(f'{OUTPUT_DIR}/regression/models', exist_ok=True)
...
handler = logging.FileHandler(f'{OUTPUT_DIR}/ML_{script_start_time}.log')
...
logger.info(f"Dropping columns: {columnKeywordsToDrop_all} and rows with missing 'flightStatusPublic'")
df_cleaned = df.dropna(subset=['flightStatusPublic']).drop(columns=df.filter(regex='|'.join(columnKeywordsToDrop_all)), errors='ignore')
...
logger.info("Filled missing 'flightLegs_irregularity_delayDuration_total' with 0")
...
if FILTER_EU:
    logger.info("Filtering flights to EU airports")
    ...
    if TOP30_EU:
        logger.info("Applying top 30 EU airports filter")
...
def get_grid_params(name, problem_type):
    if problem_type=='classification':
        ...
        if name=='Logistic_OVR':
            if GRID_LEVEL=='quick': return {'classifier__estimator__C': [0.01, 0.1, 1],'classifier__estimator__penalty': ['l2']}
            elif GRID_LEVEL=='moderate': return {'classifier__estimator__C': [0.01, 0.1, 1],'classifier__estimator__penalty': ['l2'],'classifier__estimator__solver': ['liblinear', 'lbfgs']}
...
def save_feature_importance(pipeline, pipeline_name, problem_type):
    ...
    try:
        ...
        df_imp.head(TOP_K_FEATURES).plot(kind='bar', x='feature', y='importance', legend=False, figsize=(10,6))
        ...
        top_df = df_imp.head(TOP_K_FEATURES).copy()
...
def test_pipeline_classification(name, pipeline_tuple, mode='simple'):
    ...
    if mode=='grid' and params:
        ...
        with open(f'{OUTPUT_DIR}/classification/models/{name}_{mode}.pkl','wb') as f:
            pickle.dump(pipe,f)
        logger.info(f"Fitted simple pipeline for {name} | Total duration: {time_elapsed} seconds")
...
def output_metrics_classification(name, pipe, mode='simple', processing_time=None):
    ...
    pd.DataFrame(report).transpose().to_csv(f"{OUTPUT_DIR}/classification/{name}_{mode}_classification_report.csv")
    
    # Confusion matrix
    f, ax = plt.subplots(figsize=(10,10))
    ConfusionMatrixDisplay.from_predictions(y_test_cls, y_pred, ax=ax)
    plt.savefig(f"{OUTPUT_DIR}/classification/confusion_matrix/{name}_{mode}_confusion_matrix.png")
    plt.close()
...
def output_metrics_regression(name, pipe, mode='simple', processing_time=None):
    ...
    pd.DataFrame([metrics]).to_csv(f"{OUTPUT_DIR}/regression/{name}_{mode}_regression_report.csv", index=False)
    ...
    plt.savefig(f"{OUTPUT_DIR}/regression/predictions/{name}_{mode}_yTest_predicted.png")
    plt.close()
...
df_status = df_past.head(RECORD_LIMIT)
...
if RUN_MODE == 'simple':
    classification_pipelines = {
        'Logistic_OVO':[Pipeline([
            ...
            ('classifier', OneVsOneClassifier(LogisticRegression(max_iter=1000), n_jobs=PARALLEL_JOBS))
        ...
        'RandomForest':[Pipeline([
            ('preprocessor', preprocessor_cls),
            ('classifier', RandomForestClassifier(n_jobs=PARALLEL_JOBS))
        ]), get_grid_params('RandomForest','classification')],

        'Logistic_OVR':[Pipeline([
            ('preprocessor', preprocessor_cls),
            ('classifier', OneVsRestClassifier(LogisticRegression(max_iter=1000), n_jobs=PARALLEL_JOBS))
...
if RUN_MODE == 'simple':
    regression_pipelines = {
        ...
        'RandomForestRegressor':[Pipeline([
            ('preprocessor', preprocessor_reg),
            ('classifier', RandomForestRegressor(n_jobs=PARALLEL_JOBS))
        ]), get_grid_params('RandomForestRegressor','regression')],

        'XGBRegressor':[Pipeline([
            ('preprocessor', preprocessor_reg),
            ('classifier', XGBRegressor(n_jobs=PARALLEL_JOBS))
...
regression_pipelines = {key: value for key, value in regression_pipelines.items() if key in MODEL_LIST_TO_TEST}
...
if top_features_summary:
    ...
    logger.info(f"Saved top {TOP_K_FEATURES} features summary for all pipelines")


# Global summary CSVs
if classification_summary:
    pd.DataFrame(classification_summary).to_csv(f"{OUTPUT_DIR}/classification/global_classification_summary.csv", index=False)
    logger.info("Saved global classification summary for all pipelines")

if regression_summary:
    pd.DataFrame(regression_summary).to_csv(f"{OUTPUT_DIR}/regression/global_regression_summary.csv", index=False)
    logger.info("Saved global regression summary for all pipelines")
    

if global_summary:
    pd.DataFrame(global_summary).to_csv(f"{OUTPUT_DIR}/global_ml_summary.csv", index=False)
