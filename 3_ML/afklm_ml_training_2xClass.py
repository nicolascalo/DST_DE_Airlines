...
"TARGET_CLASSIFICATION_DELAY" : "flightlegs_irregularity_delayduration_total",
"MODEL_LIST_TO_TEST" : [
    "LinearRegression",
        "RandomForest",
...
os.makedirs(f'{output_folder}/classification_status/models', exist_ok=True)
...
os.makedirs(f'{output_folder}/classification_delay/models', exist_ok=True)
...
def categorize_status(row):
    if (row[ml_training_settings['TARGET_CLASSIFICATION_STATUS']]=='CANCELLED') |(row[ml_training_settings['TARGET_CLASSIFICATION_DELAY']]>360) : 
        return 'CANCELLED'
    return 'LATE' if row[ml_training_settings['TARGET_CLASSIFICATION_DELAY']] > 0 else 'ONTIME'
...
df_cleaned[ml_training_settings['TARGET_CLASSIFICATION_DELAY']] = df_cleaned[ml_training_settings['TARGET_CLASSIFICATION_DELAY']].fillna(0)
logger.info(f"Filled missing {ml_training_settings['TARGET_CLASSIFICATION_DELAY']} with 0")
...
def categorize_delay(row):   
    delay_value = row[ml_training_settings['TARGET_CLASSIFICATION_DELAY']] 
    if delay_value < 5:
        return "]000;005]"
    if delay_value < 15:
        return "]005;015]"
...
df_delay = df_cleaned[ml_training_settings['columnKeywordsToKeep_classification_status'] +[ ml_training_settings['TARGET_CLASSIFICATION_STATUS']] + [ml_training_settings['TARGET_CLASSIFICATION_DELAY']]]

df_delay = df_delay[df_cleaned[ml_training_settings['TARGET_CLASSIFICATION_STATUS']]=='LATE'].dropna(subset=[ml_training_settings['TARGET_CLASSIFICATION_DELAY']]).drop(
...
X_cls_delay = df_delay.drop(columns=[ml_training_settings['TARGET_CLASSIFICATION_DELAY']])
y_cls_delay = df_delay[ml_training_settings['TARGET_CLASSIFICATION_DELAY']]
...
if global_summary:
    ...
    if ml_training_settings['MODEL_TO_KEEP'] == "LATEST":
        ...
        pd.concat([best_model_summary_classification_status,best_model_summary_classification_delay]).to_csv(f"{best_model_folder}/best_models.csv", index=False)

        model_files_to_delete = sorted([f for f in list(os.listdir(best_model_folder)) if ('.pkl' in f) & (best_model_summary_classification_delay_filename not in f) & (best_model_summary_classification_status_filename not in f)])
